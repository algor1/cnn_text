...
# load fron info.log
with open(os.path.join(FLAGS.loaddir, 'info.log'), 'r', encoding="utf-8") as f:
    for line in f:
        if line.find('datacount')!= -1:
            lendata= int(line[line.find('datacount')+10:])

with open(os.path.join(SAVE_DIR, 'info.log'), 'a') as f:
    f.write(str(datetime.datetime.now()))
    f.write("Continue       " + FLAGS.loaddir + '\n')
...
for epoch in range(NUM_EPOCHS):
    i=0
    for j in range(NUM_ROWS_SAVE_TO_TRAIN-NUM_ROWS_SAVE_TO_VAL, lendata, NUM_ROWS_SAVE_TO_TRAIN-NUM_ROWS_SAVE_TO_VAL ):
        x_train = np.load(os.path.join(FLAGS.loaddir, 'data_'+str(i)+'_'+str(j)+'.npy'))
        y_train = np.load(os.path.join(FLAGS.loaddir, 'labels_' + str(i) + '_' + str(j) + '.npy'))
        x_val = np.load(os.path.join(FLAGS.loaddir, 'data_'+str(j+1)+'_'+str(j+NUM_ROWS_SAVE_TO_VAL)+'.npy'))
        y_val = np.load(os.path.join(FLAGS.loaddir, 'labels_' + str(j+1) + '_' + str(j+NUM_ROWS_SAVE_TO_VAL) + '.npy'))
    ...
    x_train = np.load(os.path.join(FLAGS.loaddir, 'data_'+str(i)+'_'+str(lendata)+'.npy'))
    y_train = np.load(os.path.join(FLAGS.loaddir, 'labels_' + str(i) + '_' + str(lendata) + '.npy'))
    x_val = np.load(os.path.join(FLAGS.loaddir, 'data_' + str(lendata-1) + '_' + str(lendata) + '.npy'))
    y_val = np.load(os.path.join(FLAGS.loaddir, 'labels_' + str(lendata-1) + '_' + str(lendata) + '.npy'))
    model.fit(x_train, y_train, validation_data=(x_val, y_val), epochs=1, batch_size=100)
    model.save(os.path.join(SAVE_DIR,'CNN_woVEC'))
