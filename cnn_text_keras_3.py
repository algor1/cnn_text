#-dropout
#+ gen codes and еще нет(weight of splitted sample)
import re
import datetime
import os,sys
import numpy as np
...
from tensorflow.contrib.keras.python.keras.layers import Embedding, Conv1D, MaxPooling1D, Flatten, Dense,Dropout,concatenate
...
texts,texts_gen,labels,labels_index =read_data(filename,filename_v)
...
np.save (os.path.join(SAVE_DIR, 'word_index_gen.npy'), word_index_gen)
...
for j in range(NUM_ROWS_SAVE_TO_TRAIN-NUM_ROWS_SAVE_TO_VAL, lendata, NUM_ROWS_SAVE_TO_TRAIN-NUM_ROWS_SAVE_TO_VAL ):
    ...
    np.save(os.path.join(SAVE_DIR, 'gens_' + str(i) + '_' + str(j) + '.npy'), data_gen[i:j])
    ...
    np.save(os.path.join(SAVE_DIR, 'gens_' + str(j + 1) + '_' + str(j + NUM_ROWS_SAVE_TO_VAL) + '.npy'), data_gen[j + 1:j + NUM_ROWS_SAVE_TO_VAL])
    i=j+1
...
for epoch in range(NUM_EPOCHS):
    ...
    for j in range(NUM_ROWS_SAVE_TO_TRAIN-NUM_ROWS_SAVE_TO_VAL, lendata, NUM_ROWS_SAVE_TO_TRAIN-NUM_ROWS_SAVE_TO_VAL ):
        ...
        x_train_gen = np.load(os.path.join(SAVE_DIR, 'gens_' + str(i) + '_' + str(j) + '.npy'))
        ...
        x_val_gen = np.load(os.path.join(SAVE_DIR, 'gens_' + str(j + 1) + '_' + str(j + NUM_ROWS_SAVE_TO_VAL) + '.npy'))
        y_val = np.load(os.path.join(SAVE_DIR, 'labels_' + str(j+1) + '_' + str(j+NUM_ROWS_SAVE_TO_VAL) + '.npy'))
        model.fit([x_train, x_train_gen], y_train, validation_data=([x_val, x_val_gen], y_val), epochs=1,batch_size=100)
        model.save(os.path.join(SAVE_DIR,'CNN_woVEC'))
        i=j+1
        print("---------Epoch ",epoch,"   ------batch  ",i)
    x_train = np.load(os.path.join(SAVE_DIR, 'data_'+str(i)+'_'+str(lendata)+'.npy'))
    x_train_gen = np.load(os.path.join(SAVE_DIR, 'gens_' + str(i) + '_' + str(lendata) + '.npy'))
    ...
    x_val_gen = np.load(os.path.join(SAVE_DIR, 'gens_' + str(lendata - 1) + '_' + str(lendata) + '.npy'))
